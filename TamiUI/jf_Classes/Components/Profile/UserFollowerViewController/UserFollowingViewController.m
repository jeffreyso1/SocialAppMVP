...
- (void)viewDidLoad {
    ...
    [[TestDataManager sharedInstance] getTestFollowing:^(NSURLResponse *response, id responseObject, NSError *error) {
        if (![TMIServerUtil isError:error]) {
            ...
            if (self.pageDelegate && [self.pageDelegate respondsToSelector:@selector(didFinishFetchingFollowers:)]) {
                [self.pageDelegate didFinishFetchingFollowers:[NSString stringWithFormat:@"%lu FOLLOWING", self.followerList.count]];
            }
        }
